...
Return INSERT INTO Umowa \newline
 ...
 Values (pracownik-id, typ-umowy-id, dzial-id, stanowisko-id) \newline
	...
	\hline
	Czy-Pesel-jest-Unikalny&
	
	Sprawdza czy pesel osoby jest unikalny &
	
	PESEL : int \newline	
	&
	
ilosc-osob = (select count(*) \newline
 from osoba where PESEL=PESEL); \newline
	 \newline
	if	@ilosc-osob \> 0 \newline
		return 0 \newline
	...
	&
		\textbf{Boolean}
\begin{list}{\labelitemi}{\leftmargin=1em}
	\item true jeżeli jest unikalny
	\item false jeżeli nie jest unikalny
	\end{itemize}
	\\
\end{functionsTable}
